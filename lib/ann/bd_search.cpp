...
void ANNbd_shrink::ann_search(ANNdist box_dist, ANNmin_k	*TempANNkdPointMK, ANNpoint q, int& annptsvisited)

{
        ...
        if (ANNmaxPtsVisited != 0 && annptsvisited > ANNmaxPtsVisited) {
            ...
            cout<<ANNmaxPtsVisited<<endl;

            cout<<annptsvisited<<endl;

            cout<<TempANNkdPointMK->ith_smallest_key(0);
	...
	if (inner_dist <= box_dist) {				// if inner box is closer
                ...
                child[ANN_IN]->ann_search(inner_dist, TempANNkdPointMK, q, annptsvisited);	// search inner child first

                child[ANN_OUT]->ann_search(box_dist, TempANNkdPointMK, q, annptsvisited);	// ...then outer child

	}

	else {										// if outer box is closer

                child[ANN_OUT]->ann_search(box_dist, TempANNkdPointMK, q, annptsvisited);	// search outer child first

                child[ANN_IN]->ann_search(inner_dist, TempANNkdPointMK, q, annptsvisited);	// ...then outer child
