...
void ANNkd_tree::annkSearchMultiThread(
...

{
        ...
        ANNmin_k		*TempANNkdPointMK = new ANNmin_k(k);
        ...
        root->ann_search(annBoxDistance(q, bnd_box_lo, bnd_box_hi, dim), TempANNkdPointMK, q, annptsvisited);



        for (int i = 0; i < k; i++) {		// extract the k-th closest points

                dd[i] = TempANNkdPointMK->ith_smallest_key(i);

                nn_idx[i] = TempANNkdPointMK->ith_smallest_info(i);

        }

        delete TempANNkdPointMK;				// deallocate closest point set
...
void ANNkd_split::ann_search(ANNdist box_dist, ANNmin_k	*TempANNkdPointMK, ANNpoint			q, int& annptsvisited)

{
        ...
        if (ANNmaxPtsVisited != 0 && annptsvisited > ANNmaxPtsVisited){
            ...
            if(TempANNkdPointMK->ith_smallest_info(0) == -1){

                cout<<"maxvisited "<<ANNmaxPtsVisited<<endl;

                cout<<"visited "<<annptsvisited<<endl;
        ...
        if (cut_diff < 0) {					// left of cutting plane
                ...
                child[ANN_LO]->ann_search(box_dist, TempANNkdPointMK, q, annptsvisited);// visit closer child first
		...
		if (box_dist * ANNkdMaxErr < TempANNkdPointMK->max_key())

                        child[ANN_HI]->ann_search(box_dist, TempANNkdPointMK, q, annptsvisited);
                ...
                child[ANN_HI]->ann_search(box_dist, TempANNkdPointMK, q, annptsvisited);// visit closer child first
		...
		if (box_dist * ANNkdMaxErr < TempANNkdPointMK->max_key())
                        ...
                        child[ANN_LO]->ann_search(box_dist, TempANNkdPointMK, q, annptsvisited);
...
void ANNkd_leaf::ann_search(ANNdist box_dist, ANNmin_k		*TempANNkdPointMK, ANNpoint			q, int& annptsvisited)

{
	...
	min_dist = TempANNkdPointMK->max_key(); // k-th smallest distance so far
	...
	for (int i = 0; i < n_pts; i++) {	// check points in bucket
		...
		if (d >= ANNkdDim &&					// among the k best?
		   ...
		   (ANN_ALLOW_SELF_MATCH || dist!=0)) { // and no self-match problem
			...
			TempANNkdPointMK->insert(dist, bkt[i]);

			min_dist = TempANNkdPointMK->max_key();
